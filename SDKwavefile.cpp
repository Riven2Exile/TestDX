...
////////
HRESULT CWaveFile::QuickOpen(LPWSTR strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags,IDirectSound8 *g_Sound, IDirectSoundBuffer *g_DSBuff)
{
	DSBUFFERDESC desc;
	HRESULT hr = S_FALSE;
	...
	this->Open(strFileName, pwfx, dwFlags);


	int nLenFile = GetSize();

    byte *pBufFile = new byte[nLenFile];
    DWORD nRead = 0;
    Read(pBufFile, nLenFile, &nRead);

    desc.dwSize = sizeof(desc);
    desc.dwBufferBytes = nLenFile; // DSBSIZE_MIN 
    desc.dwReserved = 0;
    desc.guid3DAlgorithm = DS3DALG_DEFAULT;
    desc.lpwfxFormat = GetFormat();/*new WAVEFORMATEX*/;
    desc.dwFlags = DSBCAPS_CTRLPAN | DSBCAPS_CTRLVOLUME | DSBCAPS_CTRLFREQUENCY
        | DSBCAPS_GLOBALFOCUS; 

    WAVEFORMATEX    wfx ={WAVE_FORMAT_PCM, 2, 11025 , 44100, 4, 16, sizeof(WAVEFORMATEX)};
    //*(desc.lpwfxFormat) = wfx;

    //hr = g_Sound->Initialize(NULL);
    hr = g_Sound->CreateSoundBuffer(&desc, &g_DSBuff, NULL);
    void *pBuf;
    DWORD nLenBuf = 0;
    hr = g_DSBuff->Lock(0, nLenFile, &pBuf, &nLenBuf, NULL, NULL, 0L);
    memcpy(pBuf, pBufFile, nLenFile);
    hr = g_DSBuff->Unlock(pBuf, nLenBuf, NULL, 0);
    hr = g_DSBuff->Play(0, 0, 0);  // 最后一个参数如果是DSBPLAY_LOOPING(循环播放)
	delete [] pBufFile;
	return hr;
}